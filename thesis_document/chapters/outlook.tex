...
During this work, a lot of effort was put in attempting to fine-tune the models used (See \secref{sft} for details on our work, and \subref{finetune} for details on training and fine-tuning \glspl{LM} in general).
...
\todo{fix (sub)section references not resolving}

We prompted the model for both 

\section{Fine-Tuning}\label{sec:out-sft}
While attempted, we where not successfull in fine-tuning one of the selected models (See \secref{sft} for details on our work on fine-tuning, and \subref{finetune} for details on training and fine-tuning \glspl{LM} in general).
...
\section{Next-Gen Models such as GPT4}\label{sec:next-gen}

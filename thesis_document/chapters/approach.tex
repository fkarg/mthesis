...
Labels where taken from SynMOF\_A exclusively. Data from SynMOF\_A was extracted automatically, but confirmed and validated manually for correctness.
...
Instead of names from chemical compounds, the database (See \secref{data} for more information on the data source) has the \texttt{pubchempy}-\texttt{cid} (compound id) as label for solvents and additives (if applicable).

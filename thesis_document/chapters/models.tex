...
\gls{meta} released \model{llama2} a few months after \model{llama}, in which they introduced few fundamental changes (making use of \gls{GQA} for the first time), trained on more tokens and released it under a different license.
They also directly released its instruct variants.
\todo{check if I'm using instruct variant or not, or why I only have one, but two for falcon later on}

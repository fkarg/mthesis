...
\newglossaryentry{fscore}{
    name=F-Score,
    description={$ 2\cdot (precision \cdot recall) / (precision + recall)$}
}
...
\newglossaryentry{ChatGPT}{
    name=ChatGPT,
    description={A chat interface for \gls{GPT3}.5 or \gls{GPT4} from \gls{OpenAI}, which makes it easier to interface with the model. Refer to \secref{instruct} for more information on instruct-based models, chat variants are an extension of.
    }
}

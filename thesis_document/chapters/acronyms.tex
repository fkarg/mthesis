...
\newglossaryentry{causal}{
    ...
    description={A causal language model predicts the likelihood of the next token based on a sequence of tokens. By sampling one of the predicted tokens, output can be generated autoregressively. This in contrast to e.g. a \gls{masked}.}
}
...
\newglossaryentry{fscore}{
    name=F-Score,
    description={$ 2\cdot (precision \cdot recall) / (precision + recall)$, where \textit{precsision} is a metric for how many retrieved items are relevant, and \textit{recall} is a metric for how many relevant items where retrieved.}
}
...
\newglossaryentry{ChatGPT}{
    name=ChatGPT,
    description={A chat interface for \gls{GPT3}.5 or \gls{GPT4} from \gls{OpenAI}, which makes it easier to interface with the model. Refer to \subref{instruct} for more information on instruct-based models, chat variants are an extension of.
    }
}
...
\newglossaryentry{PaLM}{
    name=PaLM,
    description={An extremely capable \gls{LLM} from \gls{Google} \cite{chowdhery_palm_2022}.
    }
}
...
\newglossaryentry{llama2}{
    name=LLaMa 2,
    description={One of the \glspl{LLM} used. Created by \gls{meta}, successor of \model{llama}. See \subref{llama2} for details.
    }
}
...
\newglossaryentry{Google}{
    name=Google,
    description={Now called Alphabet, search engine giant turned AI powerhouse, well known for training supersized machine learning models with unrealistic hardware requirements for just about anyone else.}
}

\newglossaryentry{microsoft}{
    name=Microsoft,
    description={Large Tech Giant, recently starting a cooperation with \gls{OpenAI} by investing USD\$10 Billion and integrating \gls{GPT4} and other models in their services}
}
